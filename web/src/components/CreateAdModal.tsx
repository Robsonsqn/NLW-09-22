...
export function CreateAdModal() {
    ...
    const [useVoiceChanel, setUseVoiceChanel] = useState<boolean>(true)
    ...
    return (
        <Dialog.Portal>
              ...
              <Dialog.Content className='bg-[#2A2634] py-8 px-10 text-white fixed top-1/2 left-1/2 -translate-y-1/2 -translate-x-1/2 rounded-lg w-[480px] shadow-lg shadow-black/25'>
                <Dialog.Title className='text-3xl font-black'>Publique um anúncio</Dialog.Title>
                <form onSubmit={handleCreateAd} action="" className='mt-8 flex flex-col gap-4'>
                  <div className='flex flex-col gap-2'>
                    <label htmlFor="game" className='font-semibold'>Qual o game?</label>
                    {/* useSelecet from radix */}
                    <select  id='game' name='game' defaultValue="fix" className='bg-zinc-900 py-3 px-4 rounded text-sm placeholder:text-zinc-500 appearance-none'>
                        <option disabled value="fix">Selecione o game que deseja jogar</option>
                        {games.map( game => {
                  ...
                  <label className='mt-2 flex items-center gap-2 text-sm'>
                    <Checkbox.Root checked={useVoiceChanel} onCheckedChange={(checked) => { 
